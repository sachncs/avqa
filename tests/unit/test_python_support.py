...
def test_supported_python_range_matches_package_metadata() -> None:
    ...
    assert supported_python_range() == ((3, 10), (3, 16))


def test_supported_python_range_is_lower_inclusive_and_upper_exclusive() -> None:
    """The boundary accepts 3.10--3.15 and rejects adjacent versions."""
    ...
    assert is_supported_python((3, 15))
    assert not is_supported_python((3, 16))
